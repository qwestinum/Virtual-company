...
- **Pré-remplissage d'une campagne à partir d'un document (appel d'offres / notes).** Couche **commune** aux deux chemins de création — formulaire ET chat Manager : on LIT un document (distinct du Job Writer qui GÉNÈRE), on en extrait un **brouillon**, l'humain valide. **Aucune persistance avant validation.** Extraction texte PDF/**DOCX** via `extractCVText` (mammoth ajouté, server-only, partagé avec l'analyse CV) → extracteur LLM `src/lib/agents/campaign-prefill.ts` (`extractCampaignPrefill`, via `chatCompleteJson`, provider abstrait) → route commune `POST /api/campaigns/prefill` (FormData `document`). Schéma + mappings **purs** dans `src/types/campaign-prefill.ts` (`CampaignPrefill`, `prefillToFDP`, `prefillToSuggestedCriteria`, normalisation séniorité/contrat). **Le LLM n'invente jamais** (champ absent → `null`), retient la **dernière mention** sur contradiction (`conflit`). **Deux catégories** : factuels (`suggere:false`) et **pondérations suggérées** (`suggere:true`) ; les **seuils & flags éliminatoires sont hors périmètre absolu** (saisie 100% humaine — `SUGGESTABLE_LEVELS` exclut `redhibitoire`/`obligatoire`). **Garde-fou commun** : `ScoringCriterion.suggere` + `countUntreatedSuggestions` ; `campaigns-store.activateCampaign` **refuse le lancement** tant qu'une pondération suggérée n'est pas **traitée** (confirmer→`suggere:false` / rejeter→retrait), pour les **deux** chemins. Formulaire (`CampaignCreateSheet`, `ScoringDraftEditor`) : badge « Suggéré par l'IA », confirmer/rejeter + **tout confirmer/tout rejeter**, `SourceHint` au survol, `CreatedStep` bloque l'activation (« N pondérations à traiter »). Chat (`manager-flow.chooseRouteBrief`, 4ᵉ option du route-picker) : récap **proposer/décider** (`buildBriefRecap`, pur+testé) citant les extraits sources, traitement par chips (`confirmAllSuggestions`/`rejectAllSuggestions`). Traçabilité : extraction captée dans `campaigns.prefill_extraction jsonb` (nullable, **migration à appliquer manuellement**).
