...
/**
 * Liste déroulante de donneurs d'ordre avec recherche intégrée (filtre du
 * sous-onglet rapport de campagne). Option « Tous » par défaut. Les options
 * sont dérivées des campagnes chargées (pas d'appel réseau séparé).
 */

import { Check, ChevronDown, Search } from 'lucide-react';
import { useMemo, useState } from 'react';

export type DonneurOption = { id: string; label: string };
...
export function DonneurOrdreSelect({
  ...
  const selectedLabel = useMemo(
    () => options.find((o) => o.id === value)?.label ?? 'Tous',
    [options, value],
  );
