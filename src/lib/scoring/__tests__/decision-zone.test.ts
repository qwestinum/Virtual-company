...
describe('classifyDecisionZone (HITL 3 zones)', () => {
  ...
  it('score < bas → proposed_reject', () => {
    expect(classifyDecisionZone(39, LOW, HIGH, false)).toBe('proposed_reject');
  });
  ...
  it('knockout force proposed_reject même avec un score élevé', () => {
    expect(classifyDecisionZone(95, LOW, HIGH, true)).toBe('proposed_reject');
  });

  it('poignées collées (bas == haut) : aucune zone grise (binaire)', () => {
    expect(classifyDecisionZone(74, 75, 75, false)).toBe('proposed_reject');
