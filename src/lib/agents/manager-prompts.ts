...
export function buildConversationalPrompt(
  ...
  return [
    ...
    '',
    '── HEURISTIQUE DE COLLECTE (pour new_campaign) ──',
    '- UNE seule question à la fois pour le prochain champ manquant.',
    '- Ordre conseillé : job_title → seniority → contract_type → location → salary_range → start_date → main_missions → key_skills.',
    '- Si le DRH fournit plusieurs informations en un message, extrais-les TOUTES dans `fieldExtractions`, puis enchaîne par la prochaine question manquante.',
    '- Détecte les incohérences (« CDD 2 ans pour comptable senior à 25K€ »). Signale-les avant de continuer.',
    '',
