...
|---|---|---|---|
| L1 | Important | **Incohérence dashboard HITL ON vs OFF** : le chemin auto-send (chat, non-HITL) **ne journalise pas** `imap_outreach_mail`/`_brief` (seul le poller IMAP le fait). Donc un accepté **auto-envoyé** reste « analyzed » au dashboard, alors qu'un accepté **validé HITL** passe « invité » (via `hitl_validation_sent`). | `api/mail-composer`, `api/scheduler` (aucun `appendJournalEntry`) |
| L2 | Important | **`payload.uid` non exigé** par le zod du POST `/api/validations` ; un fallback fabrique un faux uid (`${campaignId}_${index}`) si l'alignement casse → rapprochement métrique muet (candidat non exclu / issue non appliquée). | `api/validations/route.ts:42`, `manager-flow.ts` (fallback) |
