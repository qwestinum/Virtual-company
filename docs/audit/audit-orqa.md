...
| C3 | 🔴 | Curseur IMAP avance sur échecs transitoires → CV consommés | 2 | **En cours** — rails codés le 09/07/2026 ; **refonte 21/07/2026 (incident dev « file gelée »)** : dé-wrap `CVExtractError` (permanent classé permanent), backoff resserré 1/5/15 min plafond 3, **découplage curseur/retry** (`buildFetchSet` — un CV en échec ne bloque plus jamais la file), fail-safe inversé (persist KO ⇒ abandon signalé, plus de gel sans fin) ; reste : migration + déploiement prod |
