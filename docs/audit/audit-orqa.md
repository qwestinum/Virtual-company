...
| C3 | 🔴 | Curseur IMAP avance sur échecs transitoires → CV consommés | 2 | **En cours** — rails codés le 09/07/2026 ; **refonte 21/07/2026 (incident dev « file gelée »)** : dé-wrap `CVExtractError` (permanent classé permanent), backoff resserré 1/5/15 min plafond 3, **découplage curseur/retry** (`buildFetchSet` — un CV en échec ne bloque plus jamais la file), fail-safe inversé (persist KO ⇒ abandon signalé, plus de gel sans fin) ; **complément 24/07/2026 (incident « re-analyses en boucle »)** : poll en 2 phases — collecte IMAP brève puis traitement HORS connexion (le socket mourait pendant les minutes d'analyse LLM, le crash avalé sautait le commit du curseur) + filet crash-net (progression committée, erreur loggée + `last_error` — SIL-7 partiellement traité) ; reste : migration + déploiement prod |
| C4 | 🔴 | CV sans fiche validée : binaire jamais stocké, pas de rescoring | 2 | **En cours** — correctif codé le 17/07/2026 (binaire stocké + file `pending_sheet` dans `imap_unmatched_cvs`, drain automatique à la validation de la fiche via hooks PUT/PATCH campagnes, cœur de rejeu partagé `unmatched-replay.ts`, garde 409 rejeu-sans-fiche) ; reste : migration (`campaign_id`/`reason`) + déploiement prod |
...
| C7 | 🔴 | Sync optimiste tasks + artifacts (pertes silencieuses UI) | 4 | **✅ Corrigé le 17/07/2026** — pattern `sync-status` répliqué : `persistTask` (res.ok vérifié, 503=démo) + registre `failedTasks`/`failedArtifacts` (artefact conservé AVEC son contenu pour le rejeu), bannière « non enregistré » étendue aux 3 registres avec réessai |
...
| C10 | 🔴 | Rapports clôturés : cap global 1000 + journal 500 → PDF client faux | 5 | **En cours** — A9/A10/A14 + surface b (KPIs Bureau exhaustifs via `fetchCandidateTotalRows`) codés le 14/07/2026, **validés à volume sur dev** (1000/200/500→exhaustif) ; A11 (sélection audit exhaustive) + A13 (fulltext borné EXPLICITE « 200 sur N ») codés le 17/07/2026 ; reste A12 (décoratif) + prod |
